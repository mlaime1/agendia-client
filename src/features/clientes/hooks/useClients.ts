...
export function useClients(): UseClientsReturn {
  ...
  const fetchClients = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = userProfile?.role === 'client'
        ? userProfile.linked_client_id
          ? [await clientsService.getById(userProfile.linked_client_id)]
          : []
  ...
  const createClient = async (data: Parameters<typeof clientsService.create>[0]) => {
    if (!permissions.can.clientCreation) throw new Error('No tienes permiso para crear clientes.');
  ...
  const updateClient = async (id: string, data: Parameters<typeof clientsService.update>[1]) => {
    if (!permissions.can.clientEditing || !permissions.canAccessClient(id)) {
